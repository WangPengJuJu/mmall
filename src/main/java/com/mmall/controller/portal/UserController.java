...
//            CookieUtil.delLoginToken(httpServletRequest,httpServletResponse);
            ShardedJedisPoolUtil.setEx(session.getId(), JsonUtil.obj2String(response.getData()),Const.RedisCacheExtime.REDIS_SESSION_EXTIME);
        }
        return response;
    }
...
//        User user = (User)session.getAttribute(Const.CURRENT_USER);
    ...
    @ResponseBody
    public ServerResponse<String> resetPassword(HttpServletRequest request,String username,String passwordNew,String passwordOld){
        String token = CookieUtil.readLoginToken(request);
        if(StringUtils.isEmpty(token)){
            return ServerResponse.createByErrorMessage("用户未登录，无法获取当前用户信息");
        }
    ...
    public ServerResponse<User> updateInfomation(HttpServletRequest request,User user){
        ...
        if(response.isSuccess()){
            ShardedJedisPoolUtil.setEx(token, JsonUtil.obj2String(response.getData()),Const.RedisCacheExtime.REDIS_SESSION_EXTIME);
        }
        return response;
    }
